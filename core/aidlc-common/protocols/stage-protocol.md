...
Directive. At Stage END, `bun {{HARNESS_DIR}}/tools/aidlc-orchestrate.ts report
...
**CWD drift warning**: If a stage runs `cd` in a shell (e.g., `cd todo-app/server && npm install`), subsequent `bun {{HARNESS_DIR}}/tools/...` calls using relative paths will fail with "Module not found". Use the Harness Adapter's project-root command form, or run `cd` commands in subshells: `(cd subdir && npm install)`.
