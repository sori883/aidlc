...
1. Inspect the active Intent:

   `pnpm exec tsx .codex/tools/aidlc-intent.ts list . --json`

2. If none exists, ask for a Scope when it was not supplied, derive a concise
   label from the user's full description, and run:

   `pnpm exec tsx .codex/tools/aidlc-intent.ts birth . "<label>" --scope <scope>`

3. Continue with the forwarding loop. Existing Intent State is authoritative;
...
`pnpm exec tsx .codex/tools/aidlc-orchestrate.ts next --project-dir .`
...
1. Initialize the diary at `memory_path`:
   ...
   `pnpm exec tsx .codex/tools/aidlc-memory.ts init --project-dir . --memory-path "<memory_path>"`
...
For every dispatched role, instruct it to read `.codex/agents/<role>.md`, then
...
1. Surface candidates:
   ...
   `pnpm exec tsx .codex/tools/aidlc-learnings.ts surface --project-dir . --slug <stage> [--unit <unit>]`
...
4. Write the confirmed version-1 selections JSON under the active Intent's
   ...
   `pnpm exec tsx .codex/tools/aidlc-learnings.ts persist --project-dir . --slug <stage> --selections-json <path> [--unit <unit>]`

5. Present the Stage outputs and ask for approval. Only a real user approval
   allows:

   `pnpm exec tsx .codex/tools/aidlc-orchestrate.ts report --project-dir . --stage <stage> --result approved [--unit <unit>]`
...
`pnpm exec tsx .codex/tools/aidlc-orchestrate.ts report --project-dir . --stage <slug> --result completed --single`
