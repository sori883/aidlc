...
`bun run --cwd .codex aidlc build-contract approve .. <candidate-sha256> <reason>`
...
`bun run --cwd .codex aidlc review approve .. <review-manifest-sha256> <reason> [human-checks.json]`
...
`bun run --cwd .codex aidlc release authorize .. <release-plan-sha256> <reason>`
...
`bun run --cwd .codex aidlc outcome decide .. <evaluation-sha256> <decision> <reason> [not-before] [deadline]`
