...
function validateProposalDecisionRule(
  ...
  deterministicApplicability?: ReviseStageExecutionPlanOptions["deterministicApplicability"],
): void {
  ...
  const deterministicallyApplicable = deterministicApplicability?.(proposal, contract) === true;
  if ((!humanDecision || !contractAllowsException) && !deterministicallyApplicable) {
    fail(
      `Core decision ${proposal.stage_id}`,
...
export function reviseStageExecutionPlan(
  ...
  const decisions = current.stage_decisions.map((existing): CoreStageDecision => {
    ...
    validateProposalDecisionRule(
      proposal,
      contracts.get(proposal.stage_id),
      options.deterministicApplicability,
    );
    return {
      schema_version: STAGE_CONTRACT_SCHEMA_VERSION,
