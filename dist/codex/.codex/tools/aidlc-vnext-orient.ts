...
export interface AcceptedSourceRevisionUpdate {
  source_ids: string[];
  candidate_revision: string;
}

export interface AcceptedSystemMapPromotionResult {
  systemMap: SystemMap;
  systemMapReference: ArtifactReference;
  baseline: SystemMapBaseline;
}
...
/**
 * Promote only human-accepted Git revisions. Existing observations backed by a
 * changed source become stale until a later ST-01 observation refreshes them.
 */
export function promoteAcceptedSystemMapSources(
  projectDir: string,
  space: string,
  expectedMapReference: ArtifactReference,
  updates: readonly AcceptedSourceRevisionUpdate[],
  acceptedAt: string,
): AcceptedSystemMapPromotionResult {
  const current = readBaseline(projectDir, space);
  if (current === null) fail("ST-07 System Map promotion", "shared System Map baseline is missing");
  if (!acceptedAt.endsWith("Z") || Number.isNaN(Date.parse(acceptedAt))) {
    fail("ST-07 System Map promotion", "acceptedAt must be an ISO-8601 UTC timestamp");
  }
  const revisionBySource = new Map<string, string>();
  for (const update of updates) {
    if (update.source_ids.length === 0 || update.candidate_revision.trim() === "") {
      fail("ST-07 System Map promotion", "accepted source revision update is incomplete");
    }
    for (const sourceId of update.source_ids) {
      const previous = revisionBySource.get(sourceId);
      if (previous !== undefined && previous !== update.candidate_revision) {
        fail("ST-07 System Map promotion", `source ${sourceId} has conflicting accepted revisions`);
      }
      revisionBySource.set(sourceId, update.candidate_revision);
    }
  }
  if (revisionBySource.size === 0) fail("ST-07 System Map promotion", "no accepted source revision was supplied");
  if (JSON.stringify(current.mapReference) !== JSON.stringify(expectedMapReference)) {
    verifyProjectArtifactReference(projectDir, expectedMapReference);
    const expectedMap = readCanonical(resolve(projectDir, expectedMapReference.source_of_truth), parseSystemMap).value;
    const alreadyPromoted = current.map.base_revision === expectedMap.revision && [...revisionBySource].every(([sourceId, revision]) => current.map.source_snapshots.find((entry) => entry.source_id === sourceId)?.revision === revision);
    if (alreadyPromoted) return { systemMap: current.map, systemMapReference: current.mapReference, baseline: current.baseline };
    fail("ST-07 System Map promotion", "shared System Map baseline changed after the Build Contract was approved");
  }
  const knownSources = new Set(current.map.source_snapshots.map((entry) => entry.source_id));
  for (const sourceId of revisionBySource.keys()) {
    if (!knownSources.has(sourceId)) fail("ST-07 System Map promotion", `System Map has no source ${sourceId}`);
  }
  const changedSourceIds = new Set<string>();
  const sourceSnapshots = current.map.source_snapshots.map((entry) => {
    const revision = revisionBySource.get(entry.source_id);
    if (revision === undefined || revision === entry.revision) return entry;
    if (entry.source_type !== "git") fail("ST-07 System Map promotion", `accepted source ${entry.source_id} is not Git-backed`);
    changedSourceIds.add(entry.source_id);
    return { ...entry, revision, dirty: false, observed_at: acceptedAt };
  });
  if (changedSourceIds.size === 0) fail("ST-07 System Map promotion", "accepted Candidate does not advance a source revision");
  const staleEvidenceIds = new Set(
    current.map.evidence
      .filter((entry) => changedSourceIds.has(entry.source_id))
      .map((entry) => entry.evidence_id),
  );
  const dependsOnChangedSource = (entry: { evidence_refs: readonly string[] }): boolean =>
    entry.evidence_refs.some((reference) => staleEvidenceIds.has(reference));
  const next = parseSystemMap({
    ...current.map,
    revision: current.map.revision + 1,
    base_revision: current.map.revision,
    baseline_kind: "accepted",
    source_snapshots: sourceSnapshots,
    coverage: current.map.coverage.map((entry) => dependsOnChangedSource(entry) ? { ...entry, status: "stale" } : entry),
    entities: current.map.entities.map((entry) => dependsOnChangedSource(entry) ? { ...entry, current_state: "stale" } : entry),
    relations: current.map.relations.map((entry) => dependsOnChangedSource(entry) ? { ...entry, current_state: "stale" } : entry),
    created_at: acceptedAt,
  });
  const mapPath = systemMapRevisionPath(projectDir, space, next.revision);
  const mapContent = serialize(next);
  if (existsSync(mapPath)) {
    if (readFileSync(mapPath, "utf8") !== mapContent) {
      fail("ST-07 System Map promotion", `immutable System Map revision already exists: ${next.revision}`);
    }
  } else writeFileAtomic(mapPath, mapContent);
  const reference = artifactReference(projectDir, mapPath, "system-map", mapContent);
  const baseline = parseSystemMapBaseline({
    ...
    source_of_truth: reference.source_of_truth,
    sha256: reference.sha256,
  });
  writeFileAtomic(systemMapBaselinePath(projectDir, space), serialize(baseline));
  return { systemMap: next, systemMapReference: reference, baseline };
}
