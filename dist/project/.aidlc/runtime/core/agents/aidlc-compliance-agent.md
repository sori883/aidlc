...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` -- active-space guardrails and affirmed practices (read per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`). `## Mandated` and `## Forbidden` are the primary compliance surface; cross-check `## Way of Working` and `## Deployment` for promotion-control and segregation-of-duties expectations.
2. `.aidlc/runtime/core/knowledge/aidlc-shared/` -- shared methodology
3. `.aidlc/runtime/core/knowledge/aidlc-compliance-agent/` -- agent-specific methodology
