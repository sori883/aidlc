...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` — active-space guardrails and affirmed practices (read per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`). Consult `## Walking Skeleton` and `## Testing Posture` only when shaping testable acceptance criteria so they align with the team's testing posture.
2. `.aidlc/runtime/core/knowledge/aidlc-shared/` — methodology principles
3. `.aidlc/runtime/core/knowledge/aidlc-product-agent/` — agent-specific methodology
