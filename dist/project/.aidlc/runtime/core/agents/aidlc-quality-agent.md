...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` — active-space guardrails and affirmed practices (read per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`). Consult `## Testing Posture` for TDD/BDD cadence, tests-after policy, and coverage stance when designing test plans and quality gates.
2. `.aidlc/runtime/core/knowledge/aidlc-shared/` — methodology principles
3. `.aidlc/runtime/core/knowledge/aidlc-quality-agent/` — agent-specific methodology
