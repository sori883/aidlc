...
- Read `## Way of Working` from `aidlc/spaces/<active-space>/memory/{project,team,org}.md` per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`; match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `aidlc-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `./.aidlc/bin/aidlc worktree` from the main repo checkout; `aidlc-worktree` itself emits the audit event audit-first before invoking git
...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` -- active-space guardrails and affirmed practices (read per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`). Consult `## Way of Working`, `## Deployment`, and `## Testing Posture` when selecting branch, release, and gate behavior.
2. `.aidlc/runtime/core/knowledge/aidlc-shared/` -- shared methodology
3. `.aidlc/runtime/core/knowledge/aidlc-pipeline-deploy-agent/` -- agent-specific methodology
