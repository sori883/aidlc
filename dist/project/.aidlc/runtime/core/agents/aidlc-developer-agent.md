...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` — active-space guardrails and affirmed practices (read per `.aidlc/runtime/core/knowledge/aidlc-shared/rules-reading.md`). Consult `## Code Style` for type-hint, formatter, linter, and team-specific conventions; follow affirmed practice over conventions inferred from a code scan.
2. `.aidlc/runtime/core/knowledge/aidlc-shared/` — methodology principles
3. `.aidlc/runtime/core/knowledge/aidlc-developer-agent/` — agent-specific methodology
