...
Load aidlc-quality-agent (lead) persona from `agents/aidlc-quality-agent.md` and knowledge from `.aidlc/runtime/core/knowledge/aidlc-quality-agent/`. Load aidlc-devsecops-agent persona from `agents/aidlc-devsecops-agent.md` and knowledge from `.aidlc/runtime/core/knowledge/aidlc-devsecops-agent/` for security testing input. Apply aidlc-quality-agent as the primary perspective with aidlc-devsecops-agent providing security testing expertise.
...
`./.aidlc/bin/aidlc orchestrate report --project-dir . --stage build-and-test --result <outcome>`.
