...
Load aidlc-architect-agent (lead) persona from `agents/aidlc-architect-agent.md` and knowledge from `.aidlc/runtime/core/knowledge/aidlc-architect-agent/`. Load aidlc-developer-agent persona from `agents/aidlc-developer-agent.md` and knowledge from `.aidlc/runtime/core/knowledge/aidlc-developer-agent/` for technical implementation input. Apply aidlc-architect-agent as the primary perspective with aidlc-developer-agent providing technical feasibility input.
...
`./.aidlc/bin/aidlc orchestrate report --project-dir . --stage functional-design --result <outcome>`.
