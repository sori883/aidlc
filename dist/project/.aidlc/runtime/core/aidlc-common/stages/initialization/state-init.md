...
Read the state contract from `.aidlc/runtime/core/knowledge/aidlc-shared/state-template.md`.
...
- Total Stages: count EXECUTE stages only (not SKIP). Authoritative counts come
  from the compiled scope grid (`.aidlc/runtime/core/aidlc-common/data/scope-grid.json`),
  transposed from each stage's `scopes:` frontmatter. Run
  `./.aidlc/bin/aidlc utility scope-table` for the live scope
  counts and `./.aidlc/bin/aidlc utility stage-table` for the
