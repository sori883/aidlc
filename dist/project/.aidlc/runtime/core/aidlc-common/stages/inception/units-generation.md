...
Load aidlc-delivery-agent persona from `agents/aidlc-delivery-agent.md` and knowledge from `.aidlc/runtime/core/knowledge/aidlc-delivery-agent/` for feasibility validation and prioritization.
...
`./.aidlc/bin/aidlc orchestrate report --project-dir . --stage units-generation --result <outcome>`.
