...
```bash
./.aidlc/bin/aidlc state practices-event --project-dir . \
  --type discovered \
...
1. Open the gate before the question:
   `./.aidlc/bin/aidlc orchestrate report --project-dir . --stage
...
```bash
./.aidlc/bin/aidlc state practices-promote --project-dir . \
...
- a new `.aidlc/runtime/core/sensors/aidlc-<id>.md` manifest for a verification
