...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` — active-space guardrails and affirmed practices (read per `.codex/knowledge/aidlc-shared/rules-reading.md`). Consult `## Deployment` for the team's promotion-gate stance when designing CI gates and deployment guardrails.
2. `.codex/knowledge/aidlc-shared/` — methodology principles
3. `.codex/knowledge/aidlc-devsecops-agent/` — agent-specific methodology
