...
1. `aidlc/spaces/<active-space>/memory/{org,team,project}.md` -- active-space guardrails and affirmed practices (read per `.codex/knowledge/aidlc-shared/rules-reading.md`). Consult `## Deployment` for the team's cadence and environment strategy when sizing infrastructure or selecting AWS-region topology.
2. `.codex/knowledge/aidlc-shared/` -- shared methodology
3. `.codex/knowledge/aidlc-aws-platform-agent/` -- agent-specific methodology
