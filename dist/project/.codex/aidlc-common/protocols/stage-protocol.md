...
2. After the learnings answer is logged: `./.codex/tools/aidlc orchestrate report --project-dir . --stage <slug> --result awaiting-approval` — the engine marks `[-]` → `[?]` and emits `STAGE_AWAITING_APPROVAL`. `/aidlc --status` now truthfully shows the held gate.
...
compiled scope grid. Use `./.codex/tools/aidlc utility
...
1. BEFORE presenting the question: `./.codex/tools/aidlc log decision --project-dir . --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `./.codex/tools/aidlc log answer --project-dir . --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
```bash
./.codex/tools/aidlc orchestrate report --project-dir . \
...
```bash
./.codex/tools/aidlc state lookup phase-of SLUG          # → phase name
./.codex/tools/aidlc state lookup next-stage SLUG SCOPE   # → next in-scope slug
./.codex/tools/aidlc state lookup agent-for SLUG          # → lead agent name
./.codex/tools/aidlc state lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
counts), and the compiled scope grid declares which stages execute. Use `./.codex/tools/aidlc utility scope-table` for the current
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   ./.codex/tools/aidlc learnings surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file and call:
   ```bash
   ./.codex/tools/aidlc learnings persist --slug <stage-slug> --selections-json <path>
   ```
...
```bash
./.codex/tools/aidlc state reuse-artifact <stage-slug> \
