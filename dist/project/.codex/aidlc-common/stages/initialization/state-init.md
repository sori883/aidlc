...
- Total Stages: count EXECUTE stages only (not SKIP). Authoritative counts come
  from the compiled scope grid (`.codex/aidlc-common/data/scope-grid.json`),
  transposed from each stage's `scopes:` frontmatter. Run
  `./.codex/tools/aidlc utility scope-table` for the live scope
  counts and `./.codex/tools/aidlc utility stage-table` for the
